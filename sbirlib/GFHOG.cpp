...
void GFHOG::ComputeImage(IplImage* src,IplImage* mask){
	...
	cvNot(edge,edge);
	IplImage*edge_tmp=cvCreateImage(cvSize(edge->widthStep+20,edge->height+20),8,1);
	cvCopyMakeBorder(edge,edge_tmp,cvPoint(11,11),IPL_BORDER_REPLICATE);
	cvReleaseImage(&edge);
	edge = edge_tmp;
...
void GFHOG::ComputeSketch(IplImage* src,IplImage* mask){
	...
	cvThreshold(runImg,runImg,245,255,CV_THRESH_BINARY);
	/*IplImage*edge_tmp=cvCreateImage(cvSize(runImg->widthStep+20,runImg->height+20),8,1);
	cvCopyMakeBorder(runImg,edge_tmp,cvPoint(11,11),IPL_BORDER_REPLICATE);
	cvReleaseImage(&runImg);
	runImg = edge_tmp;*/
...
void GFHOG::histogramOfGradients(IplImage* edge,IplImage* gradient)
{
	...
	//IplImage*edge_tmp=cvCreateImage(cvSize(edge->widthStep+20,edge->height+20),8,1);
	//cvCopyMakeBorder(edge,edge_tmp,cvPoint(11,11),IPL_BORDER_REPLICATE);
	//IplImage*gradient_tmp=cvCreateImage(cvSize(gradient->width+20,gradient->height+20),32,1);
	//cvCopyMakeBorder(gradient,gradient_tmp,cvPoint(11,11),IPL_BORDER_REPLICATE);
//	cvNot(edge,edge);
...
void GFHOG::gradientField(IplImage* inpmask32,IplImage* filtermask32)
{
	...
	for (int j=0; j<mask->height; j++) {		
		mask->imageData[j*mask->widthStep]=0;
		mask->imageData[j*mask->widthStep + (mask->width-1)]=0;
	}
	memset(mask->imageData,0,mask->widthStep);
	memset(mask->imageData+(mask->height-1)*mask->widthStep,0,mask->width);
